# 🏛️ ArtHistory — Виртуальный Музей Времени

**ArtHistory** — уникальный виртуальный музей, созданный на базе игрового движка **Unreal Engine 5**.  
Здесь каждый посетитель погружается в живую атмосферу истории, исследуя тщательно воссозданные цифровые экспонаты и путешествуя через **порталы эпох** — волшебные врата во времени, открывающие доступ к культуре и быту разных исторических периодов.

---

## ✨ Особенности проекта

- 🎨 **Интерактивные экспонаты**  
  Детально проработанные 3D-артефакты и сцены, которые можно изучать со всех сторон.

- 🌀 **Порталы времени**  
  Уникальные переходы, позволяющие перемещаться между эпохами и наблюдать развитие человечества.

- 🌍 **Полное погружение**  
  Атмосфера, звук и визуальные эффекты создают эффект присутствия в исторической реальности.

- ⚙️ **Технологии будущего**  
  Построено на передовом Unreal Engine 5, обеспечивающем максимальную реалистичность и интерактивность.

---

## 🎬 Трейлер

Оцените визуальную красоту и масштаб проекта в официальном видео-презентации:  

📺 [Смотреть трейлер ArtHistory на Rutube](https://rutube.ru/video/d39d11006686490bd10dd49435793d5c/)

---

## 🧰 Технологии

- **Unreal Engine 5** — главный движок проекта, обеспечивающий фотореалистичную графику и динамическое освещение.

---
